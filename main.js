...
const player = new PlayerRagdoll({
  world,
  scene,
  objects,
...
function animate() {
  ...
  player.body.body.angularFactor.set(0, 1, 0);
  player.head.body.angularFactor.set(0, 1, 0);

  //TODO: implement orbit camera controls
  //TODO: and player movement, but in the player class, when I make it.
  camera.position.x = player.head.position.x + Math.sin(t * 0.003) * 10;
  camera.position.z = player.head.position.z + Math.cos(t * 0.003) * 10;
  camera.lookAt(player.head.position.x, player.head.position.y, player.head.position.z);
  t++;
